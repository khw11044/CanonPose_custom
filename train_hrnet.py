...
# ----------------------------------------------------------------------------------------
        ...
        if split == 'test':
            ...
            # reprojection loss
            losses.rep = loss_weighted_rep_no_scale(inp_poses, rot_poses, gt_confidences)
            ...
            confidences_rs = gt_confidences.reshape(-1, len(all_cams), 16)
            ...
            for c_cnt in range(len(all_cams)):
                ...
                losses.view += loss_weighted_rep_no_scale(inp_poses.reshape(-1, len(all_cams), 32)[:, coi].reshape(-1, 32),
                                                        ...
                                                        gt_confidences.reshape(-1, len(all_cams), 16)[:, coi].reshape(-1, 16))
