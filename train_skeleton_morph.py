...
import networks.model_skeleton_morph as model_skeleton_morph
...
train_loader = data.DataLoader(
    my_dataset, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=0)
...
scheduler = optim.lr_scheduler.MultiStepLR(
    optimizer, milestones=[40, 80, 95], gamma=0.1)
...
for epoch in range(N_epochs):
    ...
    for i, sample in enumerate(train_loader):
        ...
        poses_2dgt = {key:sample[key+'_2dgt'] for key in all_cams}

        inp_poses = torch.zeros((poses_2d['cam0'].shape[0] * 4, 32)).cuda()
        output_poses = torch.zeros(
            (poses_2dgt['cam0'].shape[0] * 4, 32)).cuda()
        inp_confidences = torch.zeros(
            (poses_2d['cam0'].shape[0] * 4, 16)).cuda()
        ...
        if not i % 100:
            print_losses(N_epochs, epoch, i, len(my_dataset) / config.BATCH_SIZE,
                         losses_mean.__dict__, print_keys=not (i % 1000))
